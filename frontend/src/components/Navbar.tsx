...
export default function Navbar({ megaMenuProducts = {} }: NavbarProps) {
    ...
    const [activeMegaMenuCategory, setActiveMegaMenuCategory] = useState(settingsData.collections[0] || {} as any);
        ...
        <header className={styles.headerWrapper}>
                            ...
                            {isCategoryOpen && (
                                <div className={styles.customDropdownMenu}>
                                    ...
                                    <ul className={styles.dropdownList}>
                                        {settingsData.collections.map(cat => (
                                            <li key={cat.slug}>
                                                <button
            ...
            <nav className={styles.bottomSection}>
                <div className={`container ${styles.navLinks}`}>
                    ...
                    {settingsData.menus.map((menu: any) => (
                        <Link key={menu.href} href={menu.href} className={`${styles.navLink} ${menu.href === '/wholesale' ? styles.wholesaleBtn : ''}`}>{menu.label}</Link>
                    ))}
                </div>
            </nav>
            ...
            <div className={`${styles.mobileDrawer} ${isMobileMenuOpen ? styles.mobileDrawerOpen : ''}`}>
                ...
                <div className={styles.mobileDrawerBody}>
                    ...
                    {settingsData.menus.map(menu => (
                        <Link key={menu.href} href={menu.href} className={styles.mobileNavItem} onClick={() => setIsMobileMenuOpen(false)}>{menu.label}</Link>
                    ))}
                </div>
